...
- DuckMath is a Poki BUILD VARIANT, not a standalone portal: `build:duckmath` sets both `NEXT_PUBLIC_POKI=true` and `NEXT_PUBLIC_DUCKMATH=true`, inheriting all Poki behavior (accountless, relative nested-zip assets, no Playwire, no external/profile links) and diverging at exactly ONE line — the Poki SDK loader in `components/headContent.js` is skipped when `NEXT_PUBLIC_DUCKMATH==="true"`, so no ads/SDK load. All existing Poki ad hooks are gated on `window.poki`/`window.PokiSDK` (only set after the SDK script loads), so they self-disable on DuckMath. Rule: future Poki code touching an ad SDK / network / third-party script must add `&& !NEXT_PUBLIC_DUCKMATH`; code gated on `window.poki` needs nothing. `getPlatform()` checks DuckMath before Poki so telemetry reads `"duckmath"`. Build via `pnpm build:duckmath` or the `DuckMath Build` GH workflow. (Added 2026-09-12.)
