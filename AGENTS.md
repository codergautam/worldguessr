...
- Read the files you will touch. Read the files that call the files you will touch. Claude Code: use subagents for exploration so the main context stays clean. Launch them with `model: "opus"` (see Section 6).
...
- **Every background agent runs on Opus.** Pass `model: "opus"` on each subagent launch. Never leave the model unset, because the default is Fable, and Fable is not allowed for this repo.
...
- DuckMath is a Poki BUILD VARIANT, not a standalone portal: `build:duckmath` sets both `NEXT_PUBLIC_POKI=true` and `NEXT_PUBLIC_DUCKMATH=true`, inheriting Poki behavior (accountless, relative nested-zip assets, no Playwire, no profile links, no leaderboard link). It diverges in two places: the Poki SDK loader in `components/headContent.js` is skipped when `NEXT_PUBLIC_DUCKMATH==="true"` (no ads/SDK load; Google Analytics stays on), and `EMBED_SOCIAL_LINKS` / `MORE_GAMES_URL` in `components/utils/externalLinks.js` re-enable the footer Discord/YouTube buttons and add a labeled "More Games" button to duckmath.com. All existing Poki ad hooks are gated on `window.poki`/`window.PokiSDK` (only set after the SDK script loads), so they self-disable on DuckMath. Rule: future Poki code touching an ad SDK / network / third-party script must add `&& !NEXT_PUBLIC_DUCKMATH`; code gated on `window.poki` needs nothing. `getPlatform()` checks DuckMath before Poki so telemetry reads `"duckmath"`. Build via `pnpm build:duckmath` or the `DuckMath Build` GH workflow. (Added 2026-09-12.) DuckMath hosts the zip on its OWN origin and wraps index.html in a same-origin loader iframe (`pre.html` → `./index.html`), so any "am I embedded in myself" logic must key on same origin AND same pathname, never origin alone: the `MultiplayerProvider` self-embed guard did that and silently never opened the WS on DuckMath ("Not Connected to Server" on every multiplayer button). (Fixed 2026-09-17.)
- 6x portal build (Playgama Bridge v2, `NEXT_PUBLIC_6X`): the Bridge docs (wiki.playgama.com/playgama/bridge-sdk) are the source of truth. `components/PlaygamaBootstrap.js` gates the game on `bridge.initialize()` plus one storage read, and NEITHER may block play: SDK or save failure falls back to memory-only storage (`components/utils/playgamaStorage.js`, background recovery, cloud save wins as base with session changes overlaid) because Playgama moderation rejects technical messages/error screens. Platform pause/audio events are authoritative (game pauses while `isPaused`), with one deliberate exception in `components/utils/playgamaBridge.js`: a window blur caused by focusing our own Street View iframe replays the real document visibility so the SDK clears only its visibility reason. Language: settings-menu choice (`lang` via Bridge storage, only with the `wg_6x_lang_explicit` marker the settings menu sets) > `platform.language` > `en`. Every `?crazygames=` URL check goes through `isCrazyGamesHost()` in `components/utils/getPlatform.js`, which is false on 6x. Interstitials use placement `round_end`, declared in `scripts/embed-assets/6x/playgama-bridge-config.json`. No GA4 on this build. (Added 2026-09-18.)
...
## 13. WorldGuessr project overrides

These repository-specific rules are additional non-negotiables:

1. Do not auto-commit or auto-push. Commit or push only when explicitly asked.
2. Do not run web application production builds or other rebuild commands unless explicitly asked. Preserve active development servers and hot reload.
