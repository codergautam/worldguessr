...
export default function GameUI({
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (multiplayerState?.inGame && multiplayerState?.gameData?.nextEvtTime) {
        setTimeToNextMultiplayerEvt(
          Math.max(
            0,
            Math.floor(
              (multiplayerState.gameData.nextEvtTime -
                Date.now() -
                timeOffset) /
                100,
            ) / 10,
  ...
  useEffect(() => {
    if (onboarding?.nextRoundTime) {
      const interval = setInterval(() => {
        ...
        const val = Math.max(
          0,
          Math.floor((onboarding.nextRoundTime - Date.now()) / 100) / 10,
        );
        ...
        if (val === 0) {
          ...
          setOnboardingWords([text("onboardingTimeEnd")]);
  ...
  useEffect(() => {
    if (onboarding) {
      ...
      if (onboarding.round === 1) {
        ...
        setOnboardingWords([text("greatJob"), text("onboarding5")]);
        ...
        setOnboardingWords([text("onboarding10")]);
      } else if (onboarding.round === 5) {
        setOnboardingWords([text("finalRound")]);
      }
    }
  }, [onboarding?.round]);
  ...
  function guess() {
    setShowAnswer(true);
    if (showCountryButtons || setShowCountryButtons)
...
*/}
      ...
      {(!countryGuesser || (countryGuesser && showAnswer)) &&
        (!multiplayerState ||
          (multiplayerState.inGame &&
            ["guess", "getready"].includes(
              multiplayerState.gameData?.state,
            ))) &&
        (multiplayerState?.inGame && multiplayerState?.gameData?.curRound === 1
          ...
          <>
            ...
            >
              {!showAnswer && (
                <div
                  className="mapCornerBtns desktop"
                  style={{ visibility: miniMapExpanded ? "visible" : "hidden" }}
                >
                  <button
                    className="cornerBtn"
                    onClick={() => {
                  ...
                  >
                    {miniMapFullscreen ? <FaMinimize /> : <FaExpand />}
                  </button>
                  <button
                    className="cornerBtn"
                    onClick={() => {
                  ...
                  >
                    <FaThumbtack
                      style={{
                        transform: mapPinned ? "rotate(90deg)" : "rotate(0deg)",
              ...
              <div
                className={`miniMap__btns ${showAnswer ? "answerShownBtns" : ""}`}
              >
                <button
                  className={`miniMap__btn ${!pinPoint || (multiplayerState?.inGame && multiplayerState?.gameData?.players.find((p) => p.id === multiplayerState?.gameData?.myId)?.final) ? "unavailable" : ""} guessBtn`}
                  disabled={
                    !pinPoint ||
                    (multiplayerState?.inGame &&
                      multiplayerState?.gameData?.players.find(
                        (p) => p.id === multiplayerState?.gameData?.myId,
                      )?.final)
                  }
                  onClick={guess}
                >
                  ...
                  multiplayerState?.gameData?.players.find(
                    ...
                    ? multiplayerState?.gameData?.players?.reduce(
                        (acc, cur) => {
                          if (cur.final) return acc - 1;
                          return acc;
                        },
                        multiplayerState?.gameData?.players.length,
                      ) > 0
                      ? `${text("waitingForPlayers", {
                          p: multiplayerState?.gameData?.players?.reduce(
                            (acc, cur) => {
                              if (cur.final) return acc - 1;
                              return acc;
                            },
                ...
                {!multiplayerState?.inGame && (
                  <button
                    className={`miniMap__btn hintBtn ${hintShown ? "hintShown" : ""}`}
                    onClick={showHint}
                  >
                    {text("hint")}
                  </button>
            ...
            <div
              className={`mobile_minimap__btns ${miniMapShown ? "miniMapShown" : ""} ${showAnswer ? "answerShownBtns" : ""}`}
            >
              {miniMapShown && (
                <>
                  {/* guess and hint  */}
              ...
              <button
                className={`gameBtn ${miniMapShown ? "mobileMiniMapExpandedToggle" : ""}`}
                onClick={() => {
                  setMiniMapShown(!miniMapShown);
                }}
              >
                <FaMap size={miniMapShown ? 30 : 50} />
              </button>
            </div>
          </>
        )}
      ...
      {countryGuesser && otherOptions && (
        <CountryBtns
          countries={otherOptions}
          shown={!loading && showCountryButtons && !showAnswer}
          onCountryPress={(country) => {
            const isCorrect = country === latLong.country;
            if (!isCorrect && onboarding) {
              setOnboardingWords(["Not quite. Try again!"]);
              setOnboardingTextShown(true);
      ...
      {onboarding && (
        <OnboardingText
          ...
          shown={!loading && onboardingTextShown}
      ...
      <span
        className={`timer ${loading || showAnswer || !multiplayerState || (multiplayerState?.gameData?.state === "getready" && multiplayerState?.gameData?.curRound === 1) || multiplayerState?.gameData?.state === "end" ? "" : "shown"}`}
      >
        {/* Round #{multiplayerState?.gameData?.curRound} / {multiplayerState?.gameData?.rounds} - {timeToNextMultiplayerEvt}s */}
        {text("roundTimer", {
          r: multiplayerState?.gameData?.curRound,
          mr: multiplayerState?.gameData?.rounds,
          t: timeToNextMultiplayerEvt,
        })}
      </span>

      <span
        className={`timer ${loading || showAnswer || !onboarding ? "" : "shown"}`}
      >
        {/* Round #{multiplayerState?.gameData?.curRound} / {multiplayerState?.gameData?.rounds} - {timeToNextMultiplayerEvt}s */}
        {timeToNextRound
          ? text("roundTimer", {
      ...
      {multiplayerState &&
        ...
        multiplayerState?.gameData?.curRound === 1 && (
          <BannerText
            text={text("gameStartingIn", { t: timeToNextMultiplayerEvt })}
            shown={true}
          />
        )}
      ...
      <GameOptions
        singleplayer={!multiplayerState?.inGame}
        shown={gameOptionsModalShown}
        onClose={() => {
      ...
      <EndBanner
        ...
        lostCountryStreak={lostCountryStreak}
        xpEarned={xpEarned}
        usedHint={hintShown}
        session={session}
        guessed={showAnswer}
