...
export default function ModerationView({ session, setSession, inCrazyGames }) {
    const { t: text } = useTranslation("common");
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [data, setData] = useState(null);
    // Account deletion — multi-step confirm. 0 = closed, 1 = warning, 2 = type-to-confirm.
    ...
    // Final step: schedules deletion (fast — flag + 7-day grace), then signs out.
    ...
    // If the account is already within its 7-day deletion grace window, the Danger
    ...
    return (
        <div style={containerStyle}>
            ...
            {/* Danger Zone — account deletion (own account only; hidden in CrazyGames iframe).
                If a deletion is already scheduled, this becomes a Restore prompt instead. */}
            {!inCrazyGames && (
                <div style={{ ...cardStyle, border: pendingDeletion ? '1px solid rgba(255, 152, 0, 0.5)' : '1px solid rgba(220, 53, 69, 0.4)' }}>
                    {pendingDeletion ? (
                        <>
                            <h3 style={{ ...titleStyle, fontSize: 'clamp(16px, 3.5vw, 24px)', color: '#ff9800' }}>
                                {text("dangerZone")}
                            </h3>
                            <p style={{ color: '#e0e0e0', textAlign: 'center', marginBottom: '16px', fontSize: 'clamp(13px, 3vw, 15px)' }}>
                                {scheduledDeletionAt
                        ...
                        <>
                            <h3 style={{ ...titleStyle, fontSize: 'clamp(16px, 3.5vw, 24px)', color: '#ff6b6b' }}>
                                {text("dangerZone")}
                            </h3>
                            <p style={{ color: '#b0b0b0', textAlign: 'center', marginBottom: '16px', fontSize: 'clamp(12px, 2.5vw, 14px)' }}>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    <p style={{ margin: 0, color: 'rgba(255, 255, 255, 0.9)' }}>{text("deleteAccountConfirmBody", { days: 7 })}</p>
            ...
            >
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    <input
                        ...
                        style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.05)', color: 'white', fontSize: '14px', fontFamily: '"Lexend", sans-serif', boxSizing: 'border-box' }}
                    />
                </div>
            </ConfirmModal>
        </div>
