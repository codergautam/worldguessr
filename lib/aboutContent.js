...
// Google Play rating, baked into lib/storeRatingBaked.json by
// scripts/fetchStoreRating.mjs before every build, so the rating line on the
// page and the aggregateRating in the homepage JSON-LD (the "4.8 (662)"
// under the search result) follow the store. The literals are only the
// fallback if that file is ever missing or empty.
import storeRating from "@/lib/storeRatingBaked.json";
export const PLAY_STORE_RATING = {
  value: storeRating?.value || "4.8",
  count: Number(storeRating?.count) || 662,
};
