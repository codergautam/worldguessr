...
// The 6x zip has no known mount path either, so it uses the same mode.
...
// are rewritten to document-relative by scripts/packageEmbed.mjs instead.
// (GameDistribution's zip mounts at its STABLE gameId path, so build:gd sets
// NEXT_PUBLIC_BASE_PATH instead and never derives.)
...
const basePath =
  ...
  (process.env.NEXT_PUBLIC_POKI === 'true' || process.env.NEXT_PUBLIC_6X === 'true'
    ? derivePokiBase()
    : '');
