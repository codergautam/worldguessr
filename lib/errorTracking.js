...
const IGNORED = [
  ...
  '_leaflet_pos',
  // Restored Aug 2 with the Nitro revenue stopgap (Playwire swap parked on
  // branch playwire-v2).
  'NitroAds',
  // NitroPay's ad script ("scalibur") logs this when its refresh tick or a
  // queued render fires after React has already unmounted the ad slot —
  // round-end unmounts the gameUI ad, leaving home unmounts the home ad.
  // bannerAdNitro.js already destroys the slot via onNavigate(); this is the
  // SDK's own late timer noticing the container is gone. Expected noise on
  // every round end, so unfiltered it dominates GA exception events.
  'scalibur: Container not found',
  // CrazyGames SDK errors on domains where it's disabled or before it has
  // initialized ("CrazySDK is disabled on this domain", "CrazySDK is not
  // initialized yet"). Expected platform noise — every call site already
  // handles it — that otherwise floods the preceding-errors buffer and the dev
  // overlay. Our own logs around it all carry the prefix "crazygames ".
  'CrazySDK',
  'crazygames'
];
...
// Post-load document.write guard.
//
// Any document.write after the load event implicitly calls document.open(),
// which WIPES the live document — document.body is null until the parser
// rebuilds it, so React's next commit crashes ("Cannot read properties of
// null (reading 'classList')") and the app white-screens. Cross-origin ad
// iframes physically can't reach our document; the only writers are
// same-origin scripts (ad-tech identity/passback modules). No legitimate
// post-load writer exists on this site, so block the write and report the
// attempted markup — the console.error routes through reportToGA, naming
// the culprit in GA and in the fatal-webhook buffer. Blocking only arms at
// the load event, so parser-time document.write during page load is
// unaffected. Like the DOM mutation guards, installed once and never
// torn down — restoring a page-destroying primitive has no upside.
function installDocumentWriteGuard() {
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (window.__docWriteGuardInstalled) return;
  window.__docWriteGuardInstalled = true;

  const block = (name) => function () {
    const snippet = Array.prototype.join.call(arguments, '').slice(0, 200);
    console.error(`document.${name} blocked after load: ${snippet}`);
  };
  const arm = () => {
    try {
      document.write = block('write');
      document.writeln = block('writeln');
    } catch (_) { /* noop */ }
  };
...
export default function installErrorTracking() {
  ...
  const reportToGA = (description, extra = {}) => {
    ...
    const ignored = shouldIgnore(truncated);
    ...
    // IGNORED errors are buffered too (flagged, kept out of GA below): when a
    // crash comes from a cross-origin script callback the browser sanitizes it
    // to "Script error." — filtering that out of the buffer left fatal
    // webhooks with "(no preceding error captured)" and nothing to diagnose.
    if (!nextjsError) {
      errorBuffer.push({ description: truncated, extra: enrichedExtra, ignored, ts: now });
    ...
    try {
      ...
      if (nextjsError) {
        ...
        const preceding = errorBuffer.length
          ? errorBuffer
              .map((e, i) => {
                ...
                return `**[${i + 1}] (${e.extra?.error_type || 'unknown'}${e.ignored ? ', ga-ignored' : ''})**\n\`\`\`\n${e.description.slice(0, 700)}\n${evidence ? `\n\n${evidence}` : ''}\n\`\`\``;
              })
              .join('\n')
        ...
        fetch("https://discord.com/api/webhooks/1511369475231060038/osyalNpPIjTh_i92DI566YNgz18NgwbdKFasggIHAgFeOvZ08nYn2lLvfisBiS3y65Oi", {
          method: "POST",
  ...
  const suppressIfIgnored = (event) => {
    ...
    if (shouldIgnore(msg)) {
      // stopImmediatePropagation also kills our own bubble-phase handlers, so
      // route the message through reportToGA first — it dedups and buffers it
      // (flagged ga-ignored) without emitting a GA event. Without this,
      // cross-origin "Script error." crashes vanished entirely and the fatal
      // webhook shipped with an empty preceding-errors buffer.
      reportToGA(msg, {
        error_type: `${event.type || 'error'}(suppressed)`,
        source: event.filename,
      });
