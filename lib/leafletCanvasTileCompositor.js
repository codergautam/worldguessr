...
const FADE_MS = 220;
// Extra drawn margin around the viewport (css px) so pans reveal imagery
// instead of backdrop between throttled repositions. Kept small on purpose:
// the canvas backing store costs (w+2b)(h+2b)·dpr²·4 bytes, and the whole
// point of this module is to use LESS memory than the old per-tile GPU
// textures it replaces (hidden imgs composite nothing). 128 needs a pan
// faster than ~1300 css px/s to out-run the 100ms reposition cadence.
const BUFFER_PX = 128;
...
class TileCompositor {
  constructor(map) {
    ...
    this._onMove = L.Util.throttle(() => this.draw(), MOVE_THROTTLE_MS, this);
    ...
    this._eachGridLayer((layer) => {
      ...
      for (const z of order) {
        ...
        for (const tile of tiles) {
          ...
          const l = Math.round((c.x * ts.x * scale - pixelOrigin.x - origin.x) * dpr);
          const r = Math.round(((c.x + 1) * ts.x * scale - pixelOrigin.x - origin.x) * dpr);
          const t = Math.round((c.y * ts.y * scale - pixelOrigin.y - origin.y) * dpr);
          const b = Math.round(((c.y + 1) * ts.y * scale - pixelOrigin.y - origin.y) * dpr);
