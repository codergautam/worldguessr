...
// ResizeObserver is only a final-state safety net. Leaflet already follows
// window resize events; waiting for a quiet period avoids duplicating that
// work on every drag frame while still catching responsive container changes
// that land after Leaflet sampled the old size.
...
// Hand memory back when a non-viewport allocation exceeds what is needed by
// this much. Tighter than a 2x rule so minimap animation overshoot cannot
// linger: one delayed pass returns that buffer to the exact size.
...
class TileCompositor {
  constructor(map) {
    ...
    this._containerResizeTimer = null;
  ...
  _watchContainerSize() {
    ...
    this._containerResizeObserver = new ResizeObserver(() => {
      if (this._containerResizeTimer) clearTimeout(this._containerResizeTimer);
      this._containerResizeTimer = setTimeout(() => {
        this._containerResizeTimer = null;
        const map = this._map;
        if (!map || !map._loaded) return;
        ...
        if (leafletSize.x !== width || leafletSize.y !== height) {
          // Leaflet missed the final responsive-layout size. invalidateSize
          // refreshes its pixel origin, GridLayer coverage, and then emits the
          // resize event that redraws this compositor at the full width.
          map.invalidateSize({ animate: false });
          return;
        }

        // Leaflet may already have consumed the size through its window
        // listener while our canvas draw was coalesced against an earlier
        // frame. Reconcile that narrower race without another map update.
        if (this._lastW !== width + BUFFER_PX * 2
          || this._lastH !== height + BUFFER_PX * 2) {
          this.draw();
        }
  ...
  _resizeBacking(cssW, cssH, dpr) {
    ...
    if (cssW > this._allocW || cssH > this._allocH) {
      ...
      // This branch is allowed to GROW the allocation, never shrink it.
      // Browser resizes commonly trade width for height (restore/maximize,
      // orientation changes, responsive devtools). Starting both axes at the
      // content size made a height grow silently discard spare width, then the
      // reverse resize discarded spare height. That defeated the high-water
      // mark and recreated the whole GPU-backed canvas on every aspect flip.
      let allocW = Math.max(cssW, this._allocW);
      let allocH = Math.max(cssH, this._allocH);
      if (midTransition) {
    ...
    // is preserved).
    //
    // Holding more than a persistent minimap needs: give it back, but only
    // once the size has settled, so a fullscreen->corner collapse coasts too.
    // SHRINK_SLACK is deliberately tight (not a 2x rule) so geometric grow
    // overshoot is reclaimed at the first rest rather than held for the round.
    this._clearShrinkTimer();
    const cap = this._maxBackingSize();
    // A surface that fills either viewport axis is following the browser
    // viewport, not collapsing into a persistent minimap. Keep its high-water
    // mark: shrinking it after only 500ms made every window shrink/expand pair
    // destroy two full-screen backing stores and eventually left Chromium's
    // long-lived canvas layer partially grey. The allocation remains bounded
    // by the largest viewport seen by this map and is released on unmount.
    // Genuine corner/hover minimaps fill neither axis and still reclaim their
    // oversized store through the delayed path below.
    const fillsViewportAxis = cssW >= cap.x - 1 || cssH >= cap.y - 1;
    if (!fillsViewportAxis
      && (cssW * SHRINK_SLACK < this._allocW || cssH * SHRINK_SLACK < this._allocH)) {
