...
//   fluidWheelZoomLandingSpeed  zoom-levels/second touchdown rate. Once the
//                           ease decays below this speed the glide stops
//                           easing and lands ON the goal at this constant
//                           rate. Raise for a crisper stop, lower for a
//                           floatier one; see the landing-floor comment in
//                           _updateWheelZoom before setting it near 0.
L.Map.mergeOptions({
  ...
  fluidWheelZoomLandingSpeed: 0.75,
});
...
if (!L.Map.FluidWheelZoom) {
  L.Map.FluidWheelZoom = L.Handler.extend({
    ...
    _updateWheelZoom() {
      ...
      let step = remaining * (1 - Math.exp(-dt / map.options.fluidWheelZoomEase));
      // Landing floor: a pure exponential ease never arrives — after the last
      // wheel event it creeps toward the goal in ever-smaller steps for over
      // half a second. Leaflet rounds tile-level translates and marker
      // positions to WHOLE pixels every frame (with independent rounding
      // phases), so once the per-frame motion drops below ~1px the rounding
      // flips are the only visible motion: the map vibrates ±1px until the
      // creep dies out — worst on low-Hz displays where each flip persists a
      // full frame. Once the ease slows below fluidWheelZoomLandingSpeed,
      // glide at that constant rate instead and land exactly on the goal in
      // bounded time (~100ms), keeping every frame's motion above the
      // rounding noise until a dead stop.
      const floorStep = map.options.fluidWheelZoomLandingSpeed * (dt / 1000);
      if (Math.abs(step) < floorStep) step = (remaining < 0 ? -1 : 1) * floorStep;
      const zoom = Math.abs(step) >= Math.abs(remaining) ? this._goalZoom : cur + step;
