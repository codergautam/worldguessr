 ...
 *   - integer rest: the landing target is the fractional intent snapped to
 *     the nearest zoomSnap increment AT WHEEL TIME, so the glide always
 *     comes to rest on a whole zoom level — raster tiles rest at native 1:1
 *     scale instead of parking CSS-scaled with shrunken/blurry label text.
 *     One notch from rest = exactly one level. (Snapping at idle instead was
 *     tried and reverted: it moved the map after the wheel had stopped.)
  ...
  L.Map.FluidWheelZoom = L.Handler.extend({
    ...
    _onWheeling(e) {
      ...
      if (raw !== 0) {
        ...
        // reversed. zoomSnap: 0 opts a map back into free fractional rest.
        const snap = L.Browser.any3d ? map.options.zoomSnap : 1;
        let target = next;
        if (snap) {
          target = Math.round(next / snap) * snap;
