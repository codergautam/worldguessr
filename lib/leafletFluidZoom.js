 ...
 *   - grouped tile swap (see leafletGroupedTiles): once the glide LANDS,
 ...
 *     labels don't jump size several times in one scroll. (Preparing at
 *     first wheel-idle — mid-coast — was tried and reverted Aug 1: the tile
 *     DOM/decode burst stole coast frames and read as random stutter.)
...
if (!L.Map.FluidWheelZoom) {
  L.Map.FluidWheelZoom = L.Handler.extend({
    ...
    //
    // The target-level prepare used to fire HERE, at first idle — often while
    // the glide was still coasting. Its tile DOM/decode burst then landed
    // inside the coast's rAF frames: the "random stutter mid-zoom" report.
    // USER RULING Aug 1: coast smoothness beats sharpness latency, so the
    // prepare now waits for convergence (_finish) — the busy frame happens
    // with the camera at rest, where it is invisible. Sharpness arrives
    // ~200-300ms later than before; ruled acceptable.
    _onWheelIdle() {
