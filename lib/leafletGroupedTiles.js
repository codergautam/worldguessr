...
if (!L.GridLayer.prototype._groupedTiles) {
  ...
  // Ring prefetching (margin on _getTiledPixelBounds) was tried Aug 6 and
  // REVERTED: the extra fetch/decode volume dropped phones below 60fps.
  ...
  L.GridLayer.prototype._initTile = function (tile) {
    ...
    // Decode off the main thread. Leaflet never sets this, so every arriving
    // tile (512x512 at scale=2) could decode SYNCHRONOUSLY at raster time —
    // a few ms each, in bursts, landing exactly inside the reveal expand /
    // flyTo / first-hover frames (the between-rounds micro-stutters). Tiles
    // enter at opacity 0 and fade in over 220ms, so an async decode finishing
    // a frame later is invisible.
    if (tile.tagName === 'IMG') tile.decoding = 'async';
  ...
  L.GridLayer.prototype._onMoveEnd = function () {
    ...
    // ExtentFitter's smooth reveal-exit reset (components/Map.js): the
    // container shrink fires invalidateSize -> synchronous moveend every
    // frame while a flyTo is mid-flight, and both the sync below and stock
    // _update's >1-level escape respond by aborting every in-flight tile
    // fetch and rebuilding the grid — once per frame for the whole window.
    // The flight prefetches its destination level up front and clears the
    // flag at settle; 'zoom'/'viewreset' handlers stay live throughout.
    if (this._map._suspendTileUpdates) return;
    ...
    // Same ownership rule for an ACTIVE PINCH: the held-pinch preparer sets
    // the level to the gesture's directional TARGET (e.g. 6 while zoom is
    // still 5.3), while this sync derives from CURRENT zoom (5) — with
    // updateWhenIdle=false wiring throttled moves on mobile, the two fought
    // every 200ms and tiles flickered between levels during slow pinch-ins.
    // While fingers are down the preparer owns the level; finger-up's real
    // moveend still lands here and settles everything. Drag pans don't set
    // _zooming, so mid-pan tile streaming is unaffected.
    if (this._map.touchZoom?._zooming) return;

    if (this.options.updateWhenZooming === false) {
      const zoom = this._map.getZoom();
      const tileZoom = groupedTileLevelFor(zoom);
      if (this._tileZoom !== tileZoom) {
        // noPrune so the outgoing level can crossfade under the new tiles.
        // Pass the chosen level (integer) — stock _setView rounds its zoom
        // argument for level selection, which would re-minify quarter
        // rests; restore live-camera transforms for the DOM levels after.
        this._setView(this._map.getCenter(), tileZoom, true, false);
        this._setZoomTransforms(this._map.getCenter(), zoom);
...
/**
 * Tile level for a resting camera. On TOUCH devices below world zoom 5 the
 * pinch rests on quarter steps, and Math.round there picks the FINER level
 * MINIFIED (0.71-0.84x) — labels render below their designed size and the
 * map reads over-detailed (the mobile "tiny fonts, too much detail"
 * report). Google's rule, restored here: never minify at rest — show the
 * COARSER level slightly magnified, so fonts floor at native size.
 * MOBILE-ONLY on purpose: desktop wheel always RESTS on whole levels
 * (quarter rests don't exist there), so the floor bought web nothing and
 * cost it coarser-magnified tiles during mid-glide fractional states —
 * the "web looks kind of blurry" report. round() = exact pre-change web
 * behavior. Epsilon absorbs float drift so exact integers never floor
 * down a level.
 */
function groupedTileLevelFor(zoom) {
  return L.Browser.mobile && zoom < WORLD_PINCH_SNAP_MAX_ZOOM
    ? Math.floor(zoom + PINCH_SNAP_EPSILON)
    : Math.round(zoom);
}
...
export function prepareGroupedTileLevel(map, targetCenter, targetZoom) {
  ...
  map.eachLayer((layer) => {
    ...
    try {
      const roundedTarget = groupedTileLevelFor(targetZoom);
      ...
      //
      // NO map._zoom spoof here. One briefly lived (July 29) to make >1-level
      // prepares load the target level (stock _update's escape hatch re-derives
      // from map.getZoom() and bounces the prepare back to the live level).
      // It shipped and a mobile pinch-settle regression report ("ghost copy at
      // a different pan fading in, converging as the settle lands") arrived the
      // same day — this function runs on every held/quick pinch, so it was
      // reverted to keep the pinch path byte-identical to the proven embed
      // build. The reveal-exit reset flight, which genuinely needs multi-level
      // prefetch, carries its own spoofed copy in components/Map.js instead.
      // Pass the CHOSEN level (integer) rather than the raw target zoom:
      // stock _setView derives its level via Math.round, which would undo
      // the no-minify floor above. The transform restore right after re-
      // aligns the (hidden) DOM levels to the live camera as always.
      layer._setView(targetCenter, roundedTarget, true, false);
      layer._setZoomTransforms(liveCenter, liveZoom);
...
function schedulePinchPrepare(handler) {
  ...
  // Outward targets fetch IMMEDIATELY: zoom-out reveals territory no loaded
  // level covers (the jarring blue ring), parents are 4x cheaper per level,
  // and every level crossed outward becomes the ancestor-cover source for
  // the next one — the behavior stock updateWhenZooming had, which is why
  // old mobile zoom-out felt organized. Inward keeps the stability window:
  // the current level already covers the destination, so there is nothing
  // jarring to hide and skipping intermediate levels stays a win.
  const outward = target < handler._zoom;
  const delay = outward ? 0 : Math.max(
