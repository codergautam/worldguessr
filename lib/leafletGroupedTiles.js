...
/**
 * Tile level for a resting camera. Below world zoom 5 the pinch rests on
 * quarter steps, and Math.round there picks the FINER level MINIFIED
 * (0.71-0.84x) — labels render below their designed size and the map reads
 * over-detailed (the mobile "tiny fonts, too much detail" report). Google's
 * rule, restored here: never minify at rest — show the COARSER level
 * slightly magnified, so fonts have a hard floor at native size. At/above
 * zoom 5 rests are whole levels and round() is exact, so desktop and
 * deep-zoom behavior are untouched. Epsilon absorbs float drift so exact
 * integers never floor down a level.
 */
function groupedTileLevelFor(zoom) {
  return zoom < WORLD_PINCH_SNAP_MAX_ZOOM
