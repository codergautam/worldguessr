...
export function prepareGroupedTileLevel(map, targetCenter, targetZoom) {
  ...
  map.eachLayer((layer) => {
    ...
    try {
      ...
      // _zoom spoof: _setView's internal _update re-derives from map.getZoom(),
      // and when the live camera sits >1 level from the target its escape
      // hatch re-tears the grid back to the LIVE level — around the TARGET
      // center, i.e. garbage fetches and no prepared level. One-level wheel
      // prepares never hit this, but multi-level pinches and the reveal-exit
      // reset flight (z10 -> z2) always did. Synchronous block, same trick as
      // Map.js's getResetTargetForSize with _size.
      const realZoom = map._zoom;
      map._zoom = targetZoom;
      try {
