...
if (!L.GridLayer.prototype._groupedTiles) {
  ...
  L.GridLayer.prototype._onMoveEnd = function () {
    ...
    // ACTIVE PINCH: the held-pinch preparer owns the LEVEL choice (it sets
    // the gesture's directional target, e.g. 6 while zoom is still 5.3;
    // letting the sync below re-derive from current zoom made the two fight
    // every 200ms — tiles flickered between levels). But tile LOADING keeps
    // streaming with the fingers still down: stock _update at the prepared
    // level follows the moving/panning camera, so fresh tiles arrive
    // mid-gesture instead of waiting for release. Guarded to ≤1 level from
    // live because stock _update's escape hatch would otherwise re-derive
    // the level itself — the exact fight this branch exists to prevent.
    if (this._map.touchZoom?._zooming) {
      if (Math.abs(this._map.getZoom() - this._tileZoom) <= 1) {
...
// (A "never-minify" floor variant of this rounding lived here briefly for
// the compositor's mobile look and was reverted with it — on the DOM
// pipeline the stock round() level is the correct, original look.)
...
function schedulePinchPrepare(handler) {
  ...
  // Outward targets skip the stability wait: zoom-out reveals territory no
  // loaded level covers, parents are 4x cheaper per level, and each becomes
  // the ancestor-cover source for the next. But KEEP the minimum interval —
  // a fast multi-level pinch-out fired a full level swap (abort + restack +
  // fresh crossfade) at EVERY crossing, and the stacked mid-churn read as
  // tiles flickering / discontinuity. With the interval, the first outward
  // crossing prepares instantly (slow zoom-outs = every level, organized)
  // while rapid crossings coalesce to the LATEST target and skip the
  // intermediates — one clean crossfade at the destination, the original
  // grouped-tiles philosophy. Inward keeps the stability window too: the
  // current level already covers the destination.
