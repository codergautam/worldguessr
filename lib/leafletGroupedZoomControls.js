...
function updateControlZoom(map, state) {
  ...
  // Someone else wrote the camera between our frames — the pin ClickHandler
  // does a raw panBy of a whole world-width when a tap lands on a wrap copy
  // (at world zoom, most of the screen IS wrap copies). Fluid wheel ABORTS
  // on external writes, but a zoom the user explicitly requested must
  // SURVIVE them: REBASE instead — re-derive base center and the anchor
  // latlng at the same screen point from the moved camera, then continue.
  // Without this the next frame stomps the pan and anything placed relative
  // to it (the guess pin) renders 360° off until the glide ends.
  if (state.lastWrote
    && (map.getZoom() !== state.lastWrote.zoom
      || !map.getCenter().equals(state.lastWrote.center))) {
    state.baseCenter = map.getCenter();
    if (state.anchorContainerPoint) {
      state.anchorLatLng = map.containerPointToLatLng(state.anchorContainerPoint);
      state.anchorOffset = L.point(state.anchorContainerPoint)
  ...
  state.lastWrote = { center: map.getCenter(), zoom: map.getZoom() };
...
function controlZoomBy(map, delta, anchorPoint) {
  ...
  if (!state?.active) {
    ...
    if (anchorPoint) {
      state.anchorContainerPoint = L.point(anchorPoint);
