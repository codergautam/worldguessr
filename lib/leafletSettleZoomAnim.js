...
if (!L.Map.prototype._settlesZoomAnim) {
  ...
  const settle = (map) => {
    if (map && map._animatingZoom && map._onZoomTransitionEnd) {
      ...
      // A FORCED completion must broadcast unconditionally.
      // _onZoomTransitionEnd fires 'zoom' only when the animated zoom value
      // actually changed (_tempFireZoomEvent); its suppressed _move still
      // rewrites _pixelOrigin from the settle's center pan. DOM markers
      // re-derive their layer position ONLY on 'zoom' (never 'move'), so a
      // skipped broadcast leaves the guess pin offset by the settle's pan
      // delta until the next natural zoom event — the "pin offset after a
      // fast second pinch" bug (the second touch lands here via the capture
      // listener below and force-settles the first pinch's animation).
      // Firing once per forced settle is exactly what a naturally-completed
      // zoom would have done; every consumer (markers, tooltips, vectors,
      // tile compositor) resyncs to the settled camera in the same tick.
      if (!map._animatingZoom) {
        try {
          map.fire('zoom');
          map.fire('move');
        } catch (e) { /* consumer threw; state is already settled */ }
      }
    }
  };
