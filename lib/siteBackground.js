...
export function applySiteAccent(accent) {
  ...
  s.setProperty('--accWashR', String(accent.wash[0]));
  s.setProperty('--accWashG', String(accent.wash[1]));
  s.setProperty('--accWashB', String(accent.wash[2]));
  s.setProperty('--accSurfR', String(accent.surface[0]));
  s.setProperty('--accSurfG', String(accent.surface[1]));
  s.setProperty('--accSurfB', String(accent.surface[2]));
  s.setProperty('--accDeep', accent.deep);
}
...
// Paired with the animation duration in pages/_document.js. Long enough to read
// as a dissolve, short enough that a player who just signed in is not watching
// a transition instead of a menu.
export const SITE_BG_FADE_MS = 450;
 ...
 * prevent, just spread over 450ms instead of forever.
 */
