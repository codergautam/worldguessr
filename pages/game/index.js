...
export default function Game() {
    ...
    return (
        <div className={`overflow-hidden`}>
            ...
            <Header floating={true} logo={true} logoBlur={true}/>
            <Button className={"navButton fixed right-4 top-4 z-50"} onClick={() => setMultiplayerModal(true)}>Play with Friends</Button>
            ...
            <EndBanner lostCountryStreak={lostCountryStreak} guessed={guessed} latLong={latLong} pinPoint={pinPoint}
                       countryStreak={countryStreak} fullReset={fullReset} km={km}
            ...
            <div className="MainDiv">
                <div id="innerMainDiv" ref={mapDivRef}>
                    {/* loading globe */}
                    <Loader loading={loading} latLong={latLong}
                            loadingText={playingMultiplayer && multiplayerTimers?.timeTillRound ? Math.round(multiplayerTimers.timeTillRound / 1000) : 'Loading...'}/>
                    {latLong && (
                        <iframe className={`${!mapShown ? 'mapHidden' : ''} ${playingMultiplayer ? 'multiplayer' : ''}`}
                                ...
                                id="streetview"
                                style={{height: '100vh', zIndex: 10, opacity: (loading || guessed) ? '0' : ''}}
                    ...
                    {multiplayerData && (
                        <Leaderboard gameData={multiplayerData ? multiplayerData?.gameData : null}
                                     playingMultiplayer={playingMultiplayer} mobileOpen={multiplayerLeaderboardMobile}
                                     currentRound={multiplayerTimers?.currentRound - (multiplayerinMatchBuffer ? 0 : 1)}
                                     realCurrentRoundIndex={multiplayerTimers?.currentRound}
                                     gameEnded={multiplayerEnded} finish={() => {
                    ...
                    <div id="timerDiv"
                         style={{display: (playingMultiplayer && multiplayerTimers?.timeLeft && !multiplayerEnded) ? '' : 'none'}}>
                    ...
                    <div id="miniMap" onMouseEnter={() => {
                        ...
                        if (mapShown && mapFullscreen && width > 600) {
                         ...
                         className={`${guessed ? 'gameOver' : !mapShown ? 'mapHidden' : mapFullscreen ? 'mapFullscreen' : ''} ${playingMultiplayer ? 'multiplayer' : ''}`}
                         style={{visibility: loading || !latLong ? 'hidden' : ''}}>


                        <div id="mapControlsAbove"
                             style={{display: (!width || width > 600) && (!guessed) ? '' : 'none'}}>

                        </div>
                        {mapShown && latLong &&
                            <MapWidget fullscreen={mapFullscreen} pinPoint={pinPoint} setPinPoint={setPinPoint}
                                       guessed={guessed} guessing={guessing} location={latLong} setKm={setKm}
                                       height={"100%"} multiplayerSentGuess={multiplayerSentGuess}
                                       playingMultiplayer={playingMultiplayer}
                                       multiplayerGameData={multiplayerData ? multiplayerData?.gameData : null}
                                       round={multiplayerTimers?.currentRound}
                                       currentId={multiplayerData ? multiplayerData.myData.playerSecret : null}/>}
                    </div>

                    <MultiplayerModal open={multiplayerModal} close={onMultiplayerModalClose}/>

                    {pinPoint && !guessed && (
                        <button ref={guessBtnRef} className="guessBtn desktopGB" onClick={() => {
                            guess()
                        }} style={{display: width > 600 ? '' : 'none'}} disabled={loading || guessing}>
                    ...
                    }} guessing={guessing} disableDiv={guessed || loading} playingMultiplayer={playingMultiplayer}
                                  multiplayerTimers={multiplayerTimers} multiplayerRoundIndex={multiplayerRoundIndex}
                                  multiplayerinMatchBuffer={multiplayerinMatchBuffer}
                                  leaderboardClick={() => {
                                      setMultiplayerLeaderboardMobile(true);
                                      if (mapShown) {
                                          setMapShown(false);
